# TLDR Music - Spotify Charts Scraper
...
class SpotifyScraper(BaseScraper):
    """Scraper for Spotify Top 50 India playlist."""
    ...
    async def scrape(self, page) -> List[Song]:
        """Scrape Spotify Top 50 India."""
        print(f"[Spotify] Navigating to {self.url}")
        ...
        for i, row in enumerate(rows):
            try:
                ...
                songs.append(song)
                print(f"[Spotify] #{position}: {song.title} - {song.artist}")

            except Exception as e:
                print(f"[Spotify] Error parsing row {i + 1}: {e}")
                continue

        print(f"[Spotify] Scraped {len(songs)} songs")
        self.songs = songs
        return songs
